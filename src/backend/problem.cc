    ...
    case SolverType::DogLeg: {
        ...
        while (!found && (iter < iterations)) {
            ...
            while (!onestepsuccess && false_cnt < 10) {
                ...
                ComputerDoglegStep();
                // cout<<"delta norm: "<<delta_x_.norm()<<" dogleg step norm: "<<dogleg_step_.norm()<<" r: "<<radius_<<endl;
...
void Problem::MakeHessianAc() {
    ...
    for (k = edges_.begin()->first; k > k_end; --k) {
        ...
        for (i = 0; i < v_size; ++i) {
            ...
            b.segment(index_i, dim_i).noalias() -= drho * jacobian_i.transpose() * edges_[k]->Information() * edges_[k]->Residual();
        }
        }
        }
    }
...
//#endif
    for (auto &edge: edges_) {
        ...
        for (size_t i = 0; i < v_size; ++i) {
            ...
            // cout<<size<<"index i:"<<index_i<<" dim i:"<<dim_i<<" jacobian:"<<jacobian_i.rows()<<"X"<<jacobian_i.cols()<<endl;
...
bool Problem::IsGoodStepInDogLeg() {
    ...
    if (dogleg_step_ == delta_x_) {
        ...
        scale = 0.5 * alpha * (1-beta) * (1-beta) * gradient_.squaredNorm() 
                + beta * (2-beta) * currentChi_;
    }
    scale += 1e-7;
    double rho = (currentChi_ - tempChi) / scale;
